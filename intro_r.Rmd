...
R is an open source language developed by Ross Ihaka and Robert Gentleman, and is based on S, an earlier programming language used for statistical computing. Information about R's creation can be found [here](https://www.r-project.org/about.html). Today R is one of the most popular tools used in data analysis and statistics.

This class will cover a basic introduction to R. Our goals are to provide a resource for learning to use R for plant epidemiology research. In the introduction, we provide an explanation of basic but important R tools and functions to give learners the background they need for the rest of the material. Please note that additional tools and functions will also be introduced as the course progress.

There are plenty of free online resources available to help users learn R. Below are a few recommendations:
...
> **Remember:** R is a software language. Just as with learning a foreign language, it requires time and committment to learn. Once you have learned some of the basics, you will be able to continue adding new "words" and capabilities to your vocabulary and continue to build a mastery of R.
...
Users typically interact with R through RStudio, an IDE (integrated development environment). RStudio is a user-friendly software used to make it easier for people to use R. It can be reliably downloaded for free here [RStudio](https://www.rstudio.com/). The software is highly customized and includes many additional plugins(add-on pieces of code that provide additional functions). 
...
# An object can also be a character string (a series of letters or numbers within quotations), or even multiple character strings! 
d <- "R is fun"
d
...
f <- c(1,2,3,4,5) # 'c' means concatenate.
f

# Objects and numbers can be grouped into a table or data frame (more on this later):
...
# Let's start by creating a group of numbers:
integer_example <- c(1,5,7,-4)

# See if the new object is an integer-class data type:
...
Dates can be converted to POSIX format with an `as.POSIX*()` function. The * indicates that multiple options can be used here. In this case, the function can be `as.POSIXct()` or `as.POSIXlt()`, depending on what you need. You can use different time zones and date formats by defining format and tz parameters.  In the example above, we write the date as year-month-date. We can easily convert other date formats and define time zone with the as.POSIXct() function, as shown in the next example.
...
Note that we used /(slash) instead of - (dash). This is to demonstrate that R can deal with date information in many different formats that data may come in. This means that we can use different formats, and as.POSIXct() function still will print information using the default POSIX format. 

Now we will look at the difference between as.POSIX*() functions.
...
**NOTE:** There is one more component in the operation of a function: the *environment*. For the majority of the cases you don't need to use it, so we will not cover this part here. If you want to learn more, we recommend you consult Hadley Wickham's book, [Advanced R](https://adv-r.hadley.nz/functions.html).
...
The most common way to install a package is through the Comprehensive R Archive Network ([CRAN])(https://cran.r-project.org/). CRAN is a networked repository of packages.
...
We will use many of the tools available with tidyverse as we go through these educational materials. We will remind you about the different packages as we work through the course.
...
Although using the function `setwd()` will set your work directory, it is not always the best option. For example, if you work on two computers (one at home and another at work or the laboratory), or if you are collaborating with others, the work directory will need to be re-set every time the script is accessed by a new person. That is probably not very practical! 

The second way you can set a work directory properly (and avoid any issues that arise with `setwd()`) depends upon you using *good practices* for organizing and saving your work. Creating R **projects** with clear organization and defining *relative* paths is a good way to maintain a good [work flow](https://r4ds.had.co.nz/workflow-projects.html).
...
# data_demo <- readr::read_csv("D:/OneDrive_PSU/The Pennsylvania State University/Epidem class - PSU/data/intro_r/data_demo.csv")
...
Before we get into data manipulation, it is important to have a look at the data.
...
The most common type of pipe is `%>%`, which is included when you load in tidyverse and dplyr. Additionally, four different pipes are available from the [`magrittr` package](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html), and can be used to create more complicated and sophisticated piping sequences. 
...
Now that you've learned the basics of working in R, you can move into other topics like data visualization, analysis, and modelling. 